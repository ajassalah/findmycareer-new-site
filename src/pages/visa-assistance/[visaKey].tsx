...
type VisaDetailPageProps = {
  data: (typeof visaModalData)[VisaKey];
};

export const getStaticPaths: GetStaticPaths = () => ({
  paths: Object.keys(visaModalData).map((visaKey) => ({ params: { visaKey } })),
  fallback: false,
});

export const getStaticProps: GetStaticProps<VisaDetailPageProps> = ({ params }) => {
  const visaKey = params?.visaKey;

  if (typeof visaKey !== "string" || !(visaKey in visaModalData)) {
    return { notFound: true };
  }

  return {
    props: {
      data: visaModalData[visaKey as VisaKey],
    },
  };
};

export default function VisaDetailPage({ data }: VisaDetailPageProps) {
  return (
    <>
      <Head>
        <title>{data.title} | Find My Career</title>
